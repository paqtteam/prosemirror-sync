...
describe("prosemirror lib", () => {
  ...
  test("get handles missing snapshot", async () => {
    const t = convexTest(schema, modules);
    const id = crypto.randomUUID();
    const { snapshot, steps, clientIds, version } = await t.query(api.lib.get, {
      id,
      version: 2,
    });
    ...
    expect(ignoringSteps.snapshot).toEqual(null);
    expect(ignoringSteps.steps).toEqual([]);
    expect(ignoringSteps.clientIds).toEqual([]);
    expect(ignoringSteps.version).toEqual(0);
  });
  ...
  test("get skips steps before snapshot & beyond version", async () => {
    ...
    expect(snapshot).toEqual("new content");
    expect(steps).toEqual(["2", "3"]);
    expect(clientIds).toEqual([clientId, clientId2]);
    expect(version).toEqual(3);
  });
